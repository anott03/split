...
export class ListExpensesError extends Data.TaggedError("ListExpensesError")<{
    readonly cause: unknown;
}> {}

export class ListSettlementsError extends Data.TaggedError("ListSettlementsError")<{
    readonly cause: unknown;
}> {}

export class ComputeBalancesError extends Data.TaggedError("ComputeBalancesError")<{
    readonly cause: unknown;
}> {}
...
export function listExpenses(groupId: string) {
    return Effect.gen(function*() {
        const expenseRows = yield* Effect.tryPromise({
            ...
            catch: (cause) => new ListExpensesError({ cause }),
        });

        if (expenseRows.length === 0) return [];

        const splitRows = yield* Effect.tryPromise({
            try: () => db
                .select({
    ...
    return Effect.gen(function*() {
        ...
        const rows = yield* Effect.tryPromise({
            try: () => db
                ...
                .orderBy(desc(expense.createdAt)),
            catch: (cause) => new ListSettlementsError({ cause }),
        });
...
export function computeBalances(groupId: string) {
    return Effect.gen(function*() {
        const rows = yield* Effect.tryPromise({
            ...
            catch: (cause) => new ComputeBalancesError({ cause }),
        });
